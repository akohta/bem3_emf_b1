...
void eh_field_x(IMD *id,DOMD *md);
void eh_field_y(IMD *id,DOMD *md);
...
int main(int argc,char *argv[])
{
  ...
  id.rang=1.0*md.mw.lambda_0;          // range of sampling
  ...
  // x=0 plane
  eh_field_x(&id,&md);
  output_field("yz",&id,&md);
  // y=0 plane
  eh_field_y(&id,&md);
  output_field("xz",&id,&md);
  // z=0 plane
  eh_field_z(&id,&md);
...
void eh_field_x(IMD *id,DOMD *md)
{
  ...
  for(i=0;i<id->m;i++){
    ...
    for(j=0;j<id->m;j++){
      ...
      EH_t(e,h,x,id->type,md); // total field
...
void output_field(char *pl,IMD *id,DOMD *md)
{
  ...
  dt=md->mw.lambda_0/(double)id->ts;
  
  #pragma omp parallel for schedule(dynamic) 
  for(n=0;n<id->ts;n++){
    output_png(n,cexp(-I*md->mw.omega*dt*(double)n),pl,id);
  }
