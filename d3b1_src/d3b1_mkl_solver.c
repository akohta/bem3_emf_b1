        ...
        MKL_Complex8 *A,MKL_INT *xa,MKL_INT *asub,MKL_Complex8 *B)
{
  ...
  for(i=0;i<nnz;i++){
    if(fread(&tc,sizeof(double complex),1,fp)!=1){
      printf("d3b1_mkl_solver.c, d3b1_mkl_paradiso_create_matrixs(), failed to read the tc. exit...\n");
      exit(1);
    }
    A[i].real=creal(tc);
    A[i].imag=cimag(tc);
  ...
  for(j=0;j<n;j++){
    for(p=is;p<ie;p++){
      ...
      if(fread(&tc,sizeof(MKL_Complex16),1,fa)!=1){
        printf("d3b1_mkl_solver.c, d3b1_mkl_lapacke_create_matrix(), failed to read the tc. exit...\n");
        exit(1);
      }
      A[j*n+i]=tc;
    }
